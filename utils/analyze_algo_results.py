...
from utils.plot import (
    plot_multiple_best_scores_evolution_on_graph,
    plot_multiple_paretos_on_graph,
)
...
def _load_data(folder: str = r"../results") -> List[ExtractedResult]:
    extracted_results = []
    ...
    for c, file_name in enumerate(listdir(folder)):
        ...
        if ".xlsx" in file_name:
            ...
            try:
                ...
                enhanced_generation = {tuple(a[0]): a[1] for a in data}
...
if __name__ == "__main__":
    ...
    sorted_data = list(
        sorted(data, key=lambda x: x.best_fitness + x.results[-1].generation / 1000000)
    )
    ...
    plot_multiple_paretos_on_graph(
        plt, {item.name: item.generation for item in reduced_bests[0:10]}
    )
    plt.show()

    plt.figure(figsize=(15, 15))
    plot_multiple_best_scores_evolution_on_graph(
        plt, {item.name: item.results for item in reduced_bests[0:10]}, annotate=0
    )
    plt.show()
    ...
    turns_dict = defaultdict(lambda: [])

    for item in data:
        results_dict[item.name].append(
            item.best_fitness,
        )
        turns_dict[item.name].append(item.results[-1].generation)
    ...
    for name, values in results_dict.items():
        if len(values) > 10:
            ...
            avg_turns_dict[name] = sum(turns_dict[name]) / len(turns_dict[name])

    sorted_avg_results = sorted(avg_results_dict.items(), key=lambda x: x[1])[0:10]

    for avg in sorted_avg_results:
        print(avg[0], avg[1], avg_turns_dict[avg[0]])
