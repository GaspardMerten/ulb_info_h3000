...
class SimpleAlgo(INaturalSelection):
    def select_parents(self, generation: List[DNA], fitness: Dict[int, float]) -> List[int]:
        ...
        random_populace_mode = random.choices(ordered_fitness[twenty_percent_index:eighty_percent_index],
                                              k=round(ordered_fitness_length / 4))

        selected_parents_indexes = ultra_elit_mode + selected_elit_mode + random_populace_mode
    ...
    def apply_mutation(self, dna: DNA) -> DNA:
        ...
        if mutation_type < 0.2:
            ...
            dna_list[mutation_places[0]:mutation_places[1]] = reversed(dna_list[mutation_places[0]:mutation_places[1]])
    ...
    def generate_children_from_parents(self, parent_one: DNA, parent_two: DNA) -> List[DNA]:
        ...
        for switch_place in switch_places:
            ...
            parent_one_list[parent_one_list.index(parent_two_list[switch_place])] = buffer
        ...
        for switch_place in switch_places:
            ...
            parent_two_list[parent_two_list.index(parent_one_list[switch_place])] = buffer
    ...
    def get_boosted_dna(self, dna: DNA, config: GlobalConfig) -> DNA:
        ...
        for i in range(1, 20):
            for j in range(i, 20):
                ...
                dna_score = compute_total_fitness(tuple(tmp_dna), config)
