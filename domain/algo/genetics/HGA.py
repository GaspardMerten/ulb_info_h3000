...
class HGA(INaturalSelection):
    ...
    def hga_algo(self, parent_one: DNA, parent_two: DNA):
        ...
        while parent_one_length > 1:
            ...
            distance_x_k = self.config.get_distance_between(self.config.places[k], self.config.places[x])
            distance_y_k = self.config.get_distance_between(self.config.places[k], self.config.places[y])
